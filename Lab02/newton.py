...
def newton():
    ...
    for i in range(n_max-1):
        ...
        for mi in range(mi_max):
            ...
            u_next_mi = u_current_mi - ( u_current_mi - u_confirmed[i] - (dt/2)*(f(u_confirmed[i]) + f(u_current_mi)) ) / ( 1.0 - (dt/2)*(alfa - 2*beta*u_current_mi) )
    ...
    plot(t, u_confirmed, title='Newton', xlabel='t', ylabel='u(t), z(t)')
